...
export function ArticleBlock({ block }: { block: Block }) {
  switch (block.type) {
    ...
    case "details": return <details className="learning-details"><summary>{block.title}</summary><ul className="guide-points compact">{block.paragraphs.map(paragraph => <li key={paragraph}>{paragraph}</li>)}</ul>{block.code && <pre tabIndex={0} aria-label="Configuration file example"><code>{block.code}</code></pre>}{block.commands?.map(command => <CommandBlock {...command} key={command.command} />)}</details>;
