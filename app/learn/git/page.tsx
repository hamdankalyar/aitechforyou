...
export default async function GitLearningPage({ searchParams }: { searchParams: Promise<{ q?: string | string[]; view?: string | string[] }> }) {
  ...
  const results = query ? searchGitResources(resources, query) : resources.filter(resource => resource.kind === (view === "guides" ? "Guide" : "Reference"));
    ...
    <div className="git-hub-grid">
      ...
      <div className="git-catalog">
        <section aria-labelledby="catalog-heading">
          <div className="git-section-heading"><div><span className="git-label">{query ? "Search all Git resources" : view === "guides" ? "Learn the concepts" : "Browse by topic"}</span><h2 id="catalog-heading">{query ? `${results.length} ${results.length === 1 ? "result" : "results"} for “${query}”` : view === "guides" ? "Understand how Git works." : "Setup & configuration"}</h2></div>{query && <Link href="/learn/git">Clear search</Link>}</div>
          ...
          {results.map(resource => <Link className="git-result" href={resource.href} key={resource.href}><div className="git-resource-meta"><span>{resource.kind}</span>{resource.kind !== "Reference" && <span>{resource.time}</span>}</div><h3>{resource.title} <ArrowRight size={20} /></h3><p>{resource.description}</p></Link>)}
        </section>
