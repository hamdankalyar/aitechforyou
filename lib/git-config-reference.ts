...
export const gitConfigReferenceSections: {
...
}[] = [
  {
    ...
    commands: [{ command: "git config get user.name\ngit config get user.email", explanation: "Read the configured name and email. Run inside a repository to include its overrides." }],
  ...
  },
  {
    ...
    commands: [
      { command: 'git config set --global user.name "Your Name"\ngit config set --global user.email "you@example.com"', explanation: "Set or update your default commit identity." },
      { command: "git config get --global user.name\ngit config get --global user.email", explanation: "Read back only your global settings." },
    ],
  ...
  },
  {
    ...
    commands: [{ command: "git config set --global init.defaultBranch main", explanation: "Use main when initializing new repositories." }],
  ...
  },
  {
    ...
    commands: [
      { command: "git config list --global\ngit config list --local", explanation: "List your defaults, then the current repository’s settings. The local command requires a repository." },
      { command: "git config get --show-origin --show-scope user.email", explanation: "Show the effective email with the scope and file that supplied it." },
      { command: "git config get --all --show-origin --show-scope user.email", explanation: "Show all configured emails, including values hidden by an override." },
    ],
  ...
  },
  {
    ...
    commands: [{ command: "git config unset --local user.email\ngit config get user.email", explanation: "Remove the project email, then check the remaining configured value." }],
  ...
  },
  {
    ...
    commands: [
      ...
      { command: 'git config unset --local --value="Maya" --fixed-value tutorial.reader', explanation: "Remove the exact value Maya, leaving Sam." },
      { command: "git config unset --local --all tutorial.reader", explanation: "Remove every local value for this key." },
    ],
  ...
  },
  {
    ...
    commands: [{ command: 'git config set --local tutorial.topic "Git"\ngit config remove-section --local tutorial\ngit config list --local', explanation: "Create a sample key, remove its section, then inspect the remaining local settings." }],
